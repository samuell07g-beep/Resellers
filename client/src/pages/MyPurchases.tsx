      ...
      <div className="container px-4 md:px-6 py-6 md:py-12">
        <div className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            <h1 className="text-2xl md:text-3xl font-bold text-foreground mb-1">
              Minhas <span className="text-primary">Compras</span>
            </h1>
            <p className="text-xs md:text-sm text-muted-foreground">Todas as suas keys adquiridas</p>
          </div>
          <Link href="/shop" className="w-full md:w-auto">
            <Button className="w-full md:w-auto bg-primary hover:bg-primary/90 text-primary-foreground text-sm md:text-base font-medium">
        ...
        {isLoading ? (
          ...
          <div className="text-center py-12 md:py-20">
            <div className="inline-flex items-center justify-center w-14 h-14 md:w-16 md:h-16 rounded-full bg-secondary border border-border mb-3 md:mb-4">
            ...
            <Link href="/shop" className="inline-block">
              <Button className="bg-primary hover:bg-primary/90 text-primary-foreground text-sm md:text-base font-medium">
                Ver Produtos
              </Button>
            </Link>
          </div>
          ...
          <div className="space-y-4 md:space-y-6">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-2 md:gap-4">
              {[
                ...
                { label: "1 Dia", value: keys.filter((k: any) => k.days === 1).length, icon: Calendar, color: "text-foreground" },
                { label: "7 Dias", value: keys.filter((k: any) => k.days === 7).length, icon: Calendar, color: "text-primary" },
                { label: "30 Dias", value: keys.filter((k: any) => k.days === 30).length, icon: Calendar, color: "text-foreground" },
              ].map((stat, i) => (
                <div key={i} className="p-3 md:p-4 rounded-xl border border-border bg-card">
            ...
            {grouped && Object.entries(grouped).map(([variantName, variantKeys]) => (
              <div key={variantName} className="rounded-2xl border border-border bg-card overflow-hidden">
                ...
                <div className="p-3 md:p-4 space-y-2 md:space-y-3">
                  {(variantKeys as any[]).map((key: any, i: number) => (
                    <div key={key.id} className="p-3 rounded-lg border border-border bg-background/50 hover:border-primary/30 transition-colors">
